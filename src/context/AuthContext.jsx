...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      if (payload && payload.exp * 1000 > Date.now()) {
        ...
        const primaryRole = roles[0]?.role || 'TNP_SUPER_ADMIN';
  ...
  const login = async (email, password) => {
    ...
    if (!allowedAdminRoles.includes(primaryRole)) {
      throw new Error('Access denied: Candidate / Student accounts must use the Student Portal.');
    }
